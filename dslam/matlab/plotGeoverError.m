...
function plotGeoverError(decentr_state, fig_nums, label)
...
for robot_i = 1:numel(decentr_state)
    ...
    for query_i = 1:numel(r_i_data.place_matches)
        ...
        if (numel(match) > 0)
            ...
            rel_pose.gt_T_W_M = ...
                decentr_state{match.robot_i}.gt_T_W_C{match.frame_i};
            rel_pose.gt_T_W_Q = r_i_data.gt_T_W_C{query_i};
            rel_pose.gt_T_M_Q = ...
                tInv(rel_pose.gt_T_W_M) * rel_pose.gt_T_W_Q;
...
figure(fig_nums(1));
pos_error = cellfun(@(x) ...
    norm(x.Sim_M_Q(1:3, 4) - x.gt_T_M_Q(1:3, 4)), rel_poses);
subplot(1, 2, 1);
plot(pos_error);
set(gca, 'YLim', [0 10]);
