    ...
    const std::string& dataset_path, FrameVector* result) const
{
  ...
  Eigen::Matrix<int64_t, Eigen::Dynamic, 1> times_ns;
  rpg::load(timestamp_file, &times_ns);
  result->resize(times_ns.rows());

  for (int i = 0; i < times_ns.rows(); ++i)
  {
    char image_name[7];
    sprintf(image_name, "%06d", i);

    (*result)[i].time_ns = times_ns(i);
    (*result)[i].image_path =
        dataset_path + "/images/left/" + image_name + ".jpg";
    (*result)[i].image_left =
        dataset_path + "/images/left/" + image_name + ".jpg";
    (*result)[i].image_right =
        dataset_path + "/images/right/" + image_name + ".jpg";
  }
}
